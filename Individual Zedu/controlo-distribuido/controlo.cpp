...
void calc_av(node* as,float d_av_aux[],int l){
    int i=0;
    int j=0;
...
void update_lagrange(node* as,float rho,int l){
    int i=0;
    float aux1[N]={0.0};

    for(i=0;i<l;i++){
        sub_vector(as[i].d,as[i].d_av,aux1,N);
        multiply_c_vector(rho,aux1,aux1,N);
        add_vector(as[i].y,aux1,as[i].y,N);
    }
}
